...
class TemplateWidgetFormLogin {
      ...
      .appendChild(new TemplatePartRow()
        ...
        .appendChild(new TemplatePartCol()
          ...
          .appendChild(new TemplatePartForm()
            ...
            .appendChild(new TemplatePartP(this.t('Bieristo App [EO Brewer], is an <em>open source</em> system for automating your brewery with electronics boards like Rasberry PI.')))
            .appendChild(new TemplatePartDiv()
            ...
            .appendChild(new TemplatePartDiv()
              .addClass('d-grid')
              .addClass('gap-2')
              .appendChild(new TemplatePartButton(this.t('Create or Login'))
                .addVariation('primary')
                .addVariation('lg')
                .setType('submit')
              )
              .appendChild(new TemplatePartButton(this.t('Password lost'))
                .addVariation('warning')
                .addVariation('sm')
                .setAction(() => this.lostPassword())
              )
            )
  ...
  login() {
    this.app.getApi().send(
      'user/auth',
      { email: this.email.get().value, password: this.password.get().value },
  ...
  lostPassword() {
    this.app.renderLostPassword();

    return false;
  }
}

export default TemplateWidgetFormLogin;
